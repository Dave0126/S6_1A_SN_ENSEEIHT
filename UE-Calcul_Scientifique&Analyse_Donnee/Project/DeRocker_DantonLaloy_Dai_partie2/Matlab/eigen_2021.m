...
% v == 11, méthode de la puissance itérée avec déflation (à écrire)
...
switch v
    ...
    case 12
        ...
        if(flagB == 0)
            ...
            fprintf('\nTemps puissance itérée améliorée = %0.3e\n',t_v);            
            fprintf('Nombre de valeurs propres pour attendre le pourcentage = %d\n', n_evB);
            fprintf('Nombre d''itérations pour chaque couple propre\n');
            for i=1:n_evB
                fprintf('couple %d : %d\n', i, itvB(i));
            ...
            % Sauvegarde de quelques résultats
            save("power_v2", 'VB', 'WB', 'qB', 'qvB');
                       
            W = WB;
            V = VB;
        else
            if(flagB == 1)
                fprintf('puissance itérée améliorée : pourcentage %0.3e non atteint avec %d valeurs propres\n', percentage, m);
            else
                fprintf('puissance_itérée améliorée : convergence non atteinte pour un des couples propres\n')
            end
    ...
    case 2
        fprintf('\n******* calcul avec subspace iteration v2 avec p = %d ******\n', p);        
        % appel à la version 2 de la subspace iteration method
        
        t_v1 =  cputime;
        % W1 valeurs propres
        % V1 vecteurs propres
        [ W1, V1, n_ev1, it1, itv1, flag1 ] = subspace_iter_v2( A, m, percentage, p, eps, maxit );
        t_v1 = cputime-t_v1;
    ...
    case 3
        fprintf('\n******* calcul avec subspace iteration v3 avec p = %d ******\n', p);        
        % appel à la version 3 de la subspace iteration method
        
        t_v1 =  cputime;
        % W1 valeurs propres
        % V1 vecteurs propres
        [ W1, V1, n_ev1, it1, itv1, flag1 ] = subspace_iter_v3( A, m, percentage, p, eps, maxit );
        t_v1 = cputime-t_v1;
