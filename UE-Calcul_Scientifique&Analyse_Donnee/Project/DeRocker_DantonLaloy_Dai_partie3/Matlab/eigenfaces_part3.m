...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [V_2, D] = eig(Sigma2);
[D, V_2, n_ev, ~, ~, ~] = subspace_iter_v3(Sigma2, 15, 0.9, 10, 1e-8, 10000);
...
individu_moyen_masque = ones(1,n_masque)*X_masque/n_masque;

% Calcul de la matrice X_moyen de meme taille que X, 
% telle que chaque ligne contienne l'individu moyen :
X_moyen_masque = ones(n_masque,1)*individu_moyen_masque;
...
% SUBSPACE ITERATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [V_2_masque, D_masque] = eig(Sigma2);
[D_masque, V_2_masque, n_ev_masque, ~, ~, ~] = subspace_iter_v3(Sigma2, 15, 1.0, 10, 1e-8, 10000);
...
figure('Name', 'Contrastes', 'Position', [0,0,0.67*L,0.67*H]);
for q = 1:n_ev
    figure(5)
    hold on
    contraste = contraste + lambda(q) / trace;
    plot(q, contraste,'r+','MarkerSize',8,'LineWidth',2);
    xlabel('Nombre eigenfaces','FontSize',15);
    ylabel('Contraste','FontSize',15);
end
